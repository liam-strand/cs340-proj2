...
class LossyUDP(socket):
    def __init__(self):
        ...
        # make calls to socket.recvfrom timeout after one second, so that
        # self.stopped is checked
        self.settimeout(1)

    def __del__(self):
        # for our purposes, we always want to unbind the port when the app
        # stops
        super().close()

    def sendto(self, message: bytes, dst: Tuple[str, int]):
        """Unlike the sendto method provided by the BSD socket lib,
        this method never blocks (because it schedules the transmission on a
        thread).
        """
        if len(message) > 1472:
            raise RuntimeError(
        ...
        # sleep() spaces out the requests enough to eliminate reordering caused
        # by the OS process/thread scheduler.
